...
int APIENTRY WinMain(HINSTANCE p_instance,HINSTANCE p_prev_instance,LPSTR p_cmd_line,int p_show){
   ...
   g_font=new dhFastFont(&g_engine,"font.png","font.dat");
#else
   g_font=new dhFastFont(g_engine.GetEngineDevice(),"font.png","font.dat");
#endif
...
DWORD dummy1,dummy2;
   ...
   if(SUCCEEDED(g_engine.BeginScene())){
      ...
      g_font->Draw(buffer,0,0,0xFFBBBBBB);
      g_font->Draw("I have the best intentions, but the worst ideas.",0,50,0xFFFFFFFF);
      g_font->Draw("What we learn to do, we learn by doing.-Aristotle",0,90,0xFFFFFF33);
      g_font->Draw("The best mind-altering drug is truth.-Lily Tomlin",0,130,0xFF33FFFF);
      g_font->Draw("Hello World! This is a bunch of text!",0,170,0xFFFFFFFF);
