...
char* CGUIButton::GetButtonText() { return this->buttonText; }
...
void CGUIButton::Render()
{
    ...
    if (this->backgroundState) {
        core->getGraphics()->Clear(this->GetPosition().x, this->GetPosition().y,
            this->GetWidth(), this->GetHeight(), color);
        core->getGraphics()->Clear(
            this->GetPosition().x, this->GetPosition().y + this->GetHeight() - 3,
            this->GetWidth(), 3,
            core->getGraphics()->ColorMinusColor(color, 0x00333333));
    }
    RenderClip clip;
    core->getGraphics()->SetViewport(&clip, this->GetX() + 5, this->GetY(),
        this->GetWidth() - 10, this->GetHeight());

    int newX = this->GetWidth() / 2 - this->textSize / 2;
    if (this->backgroundState)
        core->getGraphics()->TextDraw(this->GetPosition().x + newX,
            this->GetPosition().y, this->GetButtonText(),
            0xffffffff);
    else
        core->getGraphics()->TextDraw(this->GetPosition().x + newX,
            this->GetPosition().y, this->GetButtonText(),
    ...
    // core->getGraphics()->GetDevice()->Clear(0, NULL, D3DCLEAR_TARGET,
    // D3DCOLOR_XRGB(0, 40, 100), 1.0f, 0); core->getGraphics()->Clear(this->GetX()
    // + newX, this->GetY(), 30, 10, 0xff00adef);
    core->getGraphics()->RestoreRenderBackup(&clip);
}

void CGUIButton::OnClick() { this->onClick = true; }
