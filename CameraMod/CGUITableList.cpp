...
void CGUITableList::AddElement(int ID, char* text)
{
    ...
    if (pointer != NULL) {
        ...
        this->scrollbox.SetUp(this->GetX(), this->GetY(), this->GetWidth(),
            this->GetHeight());
    }
    elementsCount++;
...
int CGUITableList::GetElementsCount() { return this->elementsCount; }
...
void CGUITableList::Render()
{
    ...
    if (needScrollbox) {
        RenderClip clip;
        core->getGraphics()->SetViewport(&clip, position.x, position.y,
    ...
    while (pointer != NULL) {
        if (counter == selectedID) {
            core->getGraphics()->TextDraw(
                pos.x, pos.y + (counter * 20) - this->scrollbox.GetShift(),
                pointer->text, 0xff00adef);
        } else {
            core->getGraphics()->TextDraw(
                pos.x, pos.y + (counter * 20) - this->scrollbox.GetShift(),
                pointer->text, 0xffffFFFF);
        }
        counter++;
        pointer = pointer->next;
    }
...
void CGUITableList::OnInput(unsigned short VK) { this->scrollbox.OnKey(VK); }
...
int CGUITableList::GetSelectedID() { return this->selectedID; }