...
void CGraphics::Init()
{
	...
	HFONT mono = CreateFont(15, 0, 0, 0, 0, 0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, 0, TEXT("Lucida Console"));  //arial bold, lucida console
	D3DXCreateFont(this->GetDevice(), mono, &g_mono);
	

	//HRESULT DTEX = D3DXCreateTextureFromResourceA(this->GetDevice(), GetModuleHandleA("CameraMod.dll"), MAKEINTRESOURCEA(IDB_BITMAP1), &Texture);

	HRESULT DTEX = D3DXCreateTextureFromResourceEx(this->GetDevice(), GetModuleHandleA("CameraMod.dll"), MAKEINTRESOURCEA(IDB_BITMAP1), D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_FILTER_LINEAR, D3DX_FILTER_LINEAR, D3DCOLOR_XRGB(0, 0, 0), NULL, NULL, &Texture);
	HRESULT DSPR = D3DXCreateSprite(this->GetDevice(), &Sprite);
...
void CGraphics::onEndScene()
{
		...
		if (core->getModControl()->IsActive())
		{
			if (core->getModControl()->GetState() == CMOD_PICKING)
			{
				...
				Sprite->Draw(Texture, NULL, &scaling, NULL, 0.0, &translation, D3DCOLOR_ARGB(255, 255, 255, 255));
				Sprite->End();
			}
		}
		/*char buff[10];
		...
		*/
		...
		/*for (int i = 0; i < 1000; i++)
		{
			...
			//g_mono->DrawTextA("test, realy test , just test for performance, test, test, test", -1, &rectA, DT_LEFT, 0xffFFffFF);
			//betterFont->DrawTextA(20, 30, 0xFF00ff00, "test, realy test , just test for performance, test, test, test");
		}*/
		
		//betterFont->DrawTextA(500, 200, 0xff00adef, "Was ist das");
		//betterFont->DrawTextA(500, 230, 0xff00adef, "Was ist das",D3DFONT_FILTERED);
		/*Vector3D pos;
		pos.x = 0.0f;
		pos.y = 0.0f;
		pos.z = 0.0f;
...
void	CGraphics::SetDevice(IDirect3DDevice8* device)
{
	this->device = device;
}
...
int	CGraphics::TextDrawLength(char* text)
{
	...
	g_font->DrawTextA(text, strlen(text), &recta, DT_LEFT | DT_SINGLELINE | DT_CALCRECT, 0x0);
...
void		CGraphics::SetViewport(RenderClip* backup,DWORD x, DWORD y, DWORD width, DWORD heigh)
{
	this->GetDevice()->GetTransform(D3DTS_PROJECTION, &backup->projection);
	this->GetDevice()->GetViewport(&backup->viewport);
	//int x, y;
	D3DVIEWPORT8 port;
	/*if (backup->viewport.X <= x && backup->viewport.X + backup->viewport.Width >= x)
		port.X = x;
	else
		port.X = backup->viewport.X;
	if (backup->viewport.Y <= y && backup->viewport.Y + backup->viewport.Height >= y)
		port.Y = y;
	else
	...
	if (backup->viewport.X <= sizeX && backup->viewport.X + backup->viewport.Width >= sizeX)
		port.Width = sizeX-x;
	else
		port.Width = backup->viewport.X + backup->viewport.Width - x;
	if (backup->viewport.Y <= sizeY && backup->viewport.Y + backup->viewport.Height >= sizeY)
		port.Height = sizeY - y;
	else
		port.Height = backup->viewport.Y + backup->viewport.Height - y;
*/

	if (IsInRect(x, y, backup->viewport.X, backup->viewport.Y, backup->viewport.X + backup->viewport.Width, backup->viewport.Y + backup->viewport.Height))
	{
		if (IsInRect(x+width, y+heigh, backup->viewport.X, backup->viewport.Y, backup->viewport.X + backup->viewport.Width, backup->viewport.Y + backup->viewport.Height))
		{
			port.X = x;
			port.Y = y;
			port.Width = width;
	...
	else if (IsInRect(x + width, y + heigh, backup->viewport.X, backup->viewport.Y, backup->viewport.X + backup->viewport.Width, backup->viewport.Y + backup->viewport.Height))
	{
		port.X = backup->viewport.X;
		port.Y = backup->viewport.Y;
...
void		CGraphics::DrawCubePoint(Vector3D position)
{
	...
	this->GetDevice()->SetRenderState(D3DRS_COLORWRITEENABLE,
	...
	this->GetDevice()->SetTextureStageState(0, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
	...
	this->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, qV, sizeof(QuadVertex));
}
...
void CGraphics::FillARGBGradient(int x, int y, int w, int h, D3DCOLOR color, D3DCOLOR colorSec)
{
	//const DWORD D3D_FVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1;
