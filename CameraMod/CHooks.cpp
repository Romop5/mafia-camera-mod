...
BOOL WINAPI myPeekMessage(_Out_ LPMSG lpMsg, _In_opt_ HWND hWnd,
    ...
    _In_ UINT wRemoveMsg)
{
    ...
    if (result) {
        ...
        case WM_INPUT: {
            ...
            if (core->getModControl()
                    ->IsActive()) // block whole game input when mod is active
                return false;
        }
        default:
            break;
...
{
    _asm pushad if (core->IsGamePhysicRunning())
    {
        _asm {
			XOR ESI, ESI
			jumpHere :
    ...
    _asm popad _asm { push 0x005E1050 retn; }
}
...
void GetGameSpeed(unsigned int actualSpeed, unsigned int& newspeed)
{
  ...
  if (core->getModControl()->gamespeedBox != NULL)
          desiredSpeed = (unsigned
  int)atoi(core->getModControl()->gamespeedBox->GetInput());
  ...
  newspeed = (actualSpeed * desiredSpeed / 100);*/
    ...
    if (core->getModControl()->editSettingsSpeed != NULL)
        desiredSpeed = (unsigned int)atoi(
            core->getModControl()->editSettingsSpeed->GetInput());
    // return (unsigned int) (actualSpeed*desiredSpeed/100);
    newspeed = (actualSpeed * desiredSpeed / 100);
...
{
    // hook for keyboard
    peekOriginal = (DWORD)GetProcAddress(GetModuleHandle(TEXT("user32.dll")),
        "PeekMessageW");
    InstallHook((void*)peekOriginal, myPeekMessage, peekOriginalPatch);
