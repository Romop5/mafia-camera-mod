...
void CGUIWindow::SetTitle(char* title) { sprintf(this->title, "%s", title); }
char* CGUIWindow::GetTitle() { return this->title; }

void CGUIWindow::Render()
{

    core->getGraphics()->Clear(this->GetRelativeX(), this->GetRelativeY() + 20,
        this->GetWidth(), this->GetHeight() - 20,
        0xff000000);
    RenderClip clip;
    core->getGraphics()->SetViewport(&clip, this->GetRelativeX(),
        this->GetRelativeY() + 20, this->GetWidth(),
        this->GetHeight());
    ...
    // core->getGraphics()->Clear(this->GetRelativeX(), this->GetRelativeY(),
    // this->GetWidth(), 20, 0xff00adef);
    core->getGraphics()->FillARGBGradient(this->GetRelativeX(),
        this->GetRelativeY(), this->GetWidth(),
        20, 0x7effFFff, 0x30000000);
    core->getGraphics()->TextDraw(this->GetRelativeX() + 10, this->GetRelativeY(),
        this->GetTitle(), 0xFFffFFff);

    Point2D ms = core->getGraphics()->GetMouseCoords();
    char buff[100];
    sprintf(buff, "State: %d Pos: %d %d Ms: %d %d", clickState,
        this->GetRelativeX(), this->GetRelativeY(), ms.x, ms.y);
    core->getGraphics()->TextDraw(this->GetRelativeX(), this->GetRelativeY() - 30,
        buff, 0xFFffFFff);
}
...
{
    if (IsInRect(point.x, point.y, this->GetRelativeX(), this->GetRelativeY(),
            this->GetRelativeX() + this->GetWidth(),
            this->GetRelativeY() + this->GetHeight())) {
        CGUIElementsPoolStruct* pointer = this->elements.GetFirst();
        while (pointer != 0) {
            if (pointer->element->IsVisible()) {
